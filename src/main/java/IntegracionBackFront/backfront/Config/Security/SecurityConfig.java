    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http
                ...
                .authorizeHttpRequests(auth -> auth  // Cambia authorizeRequests por authorizeHttpRequests
                        //Todos los endpoints publicos que no necesitan de una autenticacion
                        .requestMatchers(HttpMethod.POST,
                                "/api/auth/login",
                                "/api/auth/logout")
                        .permitAll() //Permitir estos
                        .requestMatchers("/api/auth/me").authenticated()
                        .requestMatchers("/api/test/admin-only").hasRole("Administrador")
                        .requestMatchers("/api/test/cliente-only").hasRole("Cliente")
                        .anyRequest().authenticated()) //Pedir autenticacion para todos estos
                .sessionManagement(sess -> sess //Proteger la cookie
                        .sessionCreationPolicy(SessionCreationPolicy.STATELESS)) //Evitar que cambien la cookie
